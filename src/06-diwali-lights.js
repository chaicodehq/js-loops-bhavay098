...
export function diwaliLightsPlan(lightStrings, budget) {
  ...
  for (let light of lightStrings) {
    ...
    if (light.color === "golden") cost = light.length * 50;
    else if (light.color === "multicolor") cost = light.length * 40;
    else if (light.color === "white") cost = light.length * 30;
    else cost = light.length * 35;
  ...
  while (totalCost > budget) {
    totalLength -= selected[selected.length - 1].length;
    totalCost -= selected[selected.length - 1].cost;
    selected.pop();
  }
